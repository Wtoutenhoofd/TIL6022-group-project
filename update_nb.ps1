...
$nb = Get-Content 'project_template.ipynb' -Raw | ConvertFrom-Json
for ($i = 0; $i -lt $nb.cells.Count; $i++) {
    if ($nb.cells[$i].cell_type -eq "markdown") {
        $source = $nb.cells[$i].source -join ""
        if ($source -match "# 3\. Data used") {
            Write-Host "Found section 3 at cell $i"
            $newContent = "# 3. Data Used`n`n" + $dataContent
            $nb.cells[$i].source = $newContent -split "`n" | ForEach-Object { $_ + "`n" }
            break
        }
    }
}
$nb | ConvertTo-Json -Depth 100 | Set-Content 'project_template.ipynb' -Encoding UTF8
Write-Host "Successfully updated project_template.ipynb!"
